Coffeemaker.core.capacities.toolchain.make = function(thing) {
		...
		var configure = load('toolchain/configure');
        var log = load('log');             
		...
		if ( check.isHTML(thing) ) { 
		    ...
		    return thing;
		        ...
		        if( ! thing.match(/^system::/) && check.objectSize(repo[thing]) !== 0) {
		        	//thing = compile(repo[thing]);
		        	thing = configure(repo[thing]);
                    log.exception('make.js','Make use a '+thing+' found in the component repo');
		        }else{
		            if( check.objectSize(repo[thing]) !== 0 ){ 
                        log.exception('make.js','Make create a system '+thing+' because it could not find one in the component repo');
                    }
                    log.exception('make.js','Make create a '+thing+' because it could not find one in the component repo');
